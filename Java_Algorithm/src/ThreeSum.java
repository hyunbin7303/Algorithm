...
public class ThreeSum {
    ...
    public static void printAll(int[] a) {
        int n = a.length;
        int count = 1; // used for counting line num
        for (int i = 0; i < n; i++) {
            for (int j = i+1; j < n; j++) {
                for (int k = j+1; k < n; k++) {
                    if (a[i] + a[j] + a[k] == 0) {
                        StdOut.println("[ "+ count+"] "+ a[i] + " " + a[j] + " " + a[k]);
                        count++;
                    }
                }
            }
...
/*
Three Sum result 
1Kints.txt - elapsed time 0.95 - Find 70
2Kints.txt - elapsed time 6.738 - Find 528
4Kints.txt - elapsed time 53.082 - Find 4039
8Kints.txt - elapsed time 356.481 - Find 32074
